class EventsController < ApplicationController
  ...
  def register_user
    ...
    if @user.nil? || @event.users.include?(@user)
      redirect_to register_to_event_path(@event), notice: 'No se puede agregar el usuario al evento'
      return
    end

    @event.users << @user
    redirect_to register_to_event_path(@event), notice: 'El usuario se agregó correctamente'
  end
  ...
  private
    ...
    def is_event_full?
      if @event.is_event_full?
        redirect_to register_to_event_path(@event), notice: "El evento: #{@event.name} está lleno"
      end
    end

    def is_valid_email?
      if params[:email].blank? || params[:email] !~ User::VALID_EMAIL_REGEX
        redirect_to register_to_event_path(@event), notice: "Debes especificar un correo electrónico valido"
      end
    end
end
